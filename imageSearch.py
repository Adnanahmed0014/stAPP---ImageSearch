...
def _search_by_vector(v, n: int):
        ...
        #ORIGINAL return dict(zip(I[0], image_data.iloc[I[0]]['images_paths'].to_list()))
        #New: 
        #return [os.path.abspath(path) for path in image_data.iloc[I[0]]['images_paths'].to_list()]
        image_paths = [os.path.abspath(path) for path in image_data.iloc[I[0]]['images_paths'].to_list()]
        # Remove the /drive/MyDrive/ part from the file path
        image_paths = [path.replace('/ImageSearch/drive/MyDrive/', '/') for path in image_paths]  # Assuming /drive/MyDrive/ is the part to remove
        return image_paths
...
def plot_similar_images_new(image_path: str, number_of_images: int = 6):
        ...
        #ORIGINAL: img_list = list(_search_by_vector(query_vector, number_of_images).values())
        #New: 
